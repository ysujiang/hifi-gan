...
class AudioProcessor(object):
    def __init__(self,
        ...
        self.inv_mel_basis = np.linalg.pinv(self._build_mel_basis(self.mel_fmax))
        # setup scaler
        if stats_path:
    ...
    def _build_mel_basis(self, mel_fmax):
    ...
    def _normalize(self, S):
        ...
        if self.signal_norm:#true
            ...
            if self.symmetric_norm:#true
                S_norm = ((2 * self.max_norm) * S_norm) - self.max_norm
                if self.clip_norm:#true将其限制在-4到4之间，当大于4时，为4，当小于-4时为-4
    ...
    def _amp_to_db(self, x):
        return self.spec_gain * np.log10(np.maximum(1e-5, x))#x与1e-5逐个比较，选择最大的。spec_gain=20
    ...
    ### SPECTROGRAMs ###
    def _linear_to_mel(self, spectrogram,mel_max):
        mel_basis = self._build_mel_basis(mel_max)
        return np.dot(mel_basis, spectrogram)#做点积，self.mel_basis梅尔系数
    ...
    def melspectrogram(self, y,mel_max):
        if self.preemphasis != 0:
            D = self._stft(self.apply_preemphasis(y))
        else:
            D = self._stft(y)
        S = self._amp_to_db(self._linear_to_mel(np.abs(D),mel_max))##_amp_to_db变成db，将线性普转化为mel谱
        return self._normalize(S)
    ...
    def trim_silence(self, wav):
        ...
        wav = wav[margin:-margin]##切除前后0.01s的语音，前后220个点被移除
    ...
    def load_wav(self, filename, sr=None):
        ...
        # print('before_abs',max(abs(x)))
        # print('max',max(x))
        if self.do_sound_norm:
            x = self.sound_norm(x)
        # print('after_abs',max(abs(x)))
        # print('max_after',max(x))
        # print('+++++++++++++++')
        return x
