...
class MelDataset(torch.utils.data.Dataset):
    def __init__(self, training_files, ap,segment_size, n_fft, num_mels,
        ...
        self.feat_frame_len = segment_size//hop_size
    ...
    def __getitem__(self, index):
        ...
        if not self.fine_tuning:
            ...
            if self.split:
                if audio.size(1) < self.segment_size:
                    #print('---------------------------------')
                    audio = torch.nn.functional.pad(audio, (0, self.segment_size - audio.size(1)+self.hop_size*2), 'constant')
                audio = audio.squeeze(0)
                mel = self.ap.melspectrogram(audio, self.fmax)
                mel_loss = self.ap.melspectrogram(audio, self.fmax_loss)
                max_mel_start = mel.shape[1] - self.feat_frame_len-1
                ...
                #print(audio.size(0))
                #if audio.size(0)!=8192:
                    #print(filename)
                    #print(self.feat_frame_len)
            else:
                audio = audio.squeeze(0)
                mel = self.ap.melspectrogram(audio, self.fmax)
                mel_loss = self.ap.melspectrogram(audio, self.fmax_loss)
